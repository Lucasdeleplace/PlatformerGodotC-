using Godot;public partial class Enemy : CharacterBody2D
{
    ...
    public void Die()
    {
        if (_Special)
        {
            ...
            if (Health <= 0)
            {
                QueueFree(); // Supprime l'ennemi si sa vie est épuisée
            }
        }
        else
        {
            QueueFree(); // Supprime l'ennemi immédiatement s'il n'est pas spécial
        }
    }
}